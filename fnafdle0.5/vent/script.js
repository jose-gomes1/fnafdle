...
function updateMangleStatus() {
    mangleStatusElement.textContent = `Springtrap distance: ${mangleDistance}`;
}
...
function repairAction() {
    if (Math.random() < 0.45) {  // 45% chance to successfully repair
        ...
        mangleDistance++;
        updateMangleStatus();
        updateMessage("Repair failed! Springtrap is getting closer.");
    }
    ...
    if (mangleDistance >= maxMangleDistance) {
        gameOver("Springtrap got to you! Game over.");
    }
}
...
function checkAction() {
    if (mangleDistance > 0) {
        mangleDistance--;
        updateMangleStatus();
        updateMessage("You checked the vents and saw Springtrap. He has been pushed back.");
    } else {
        updateMessage("The vents are clear. No sign of Springtrap.");
    }
...
function startGame() {
    ...
    updateTasks();
    updateMangleStatus();
    updateMessage("Repair the vents while keeping an eye on Springtrap");
