...
const animatronics = [
    { name: "Bonnie the Bunny", cost: 30, initialCost: 30, pps: 1, img: "images/bonnie.png", sounds: ["sounds/bonnie1.oga", "sounds/bonnie2.oga", "sounds/bonnie3.oga"], owned: 0 },
    { name: "Chica the Chicken", cost: 200, initialCost: 200, pps: 5, img: "images/chica.png", sounds: ["sounds/chica.mp3"], owned: 0 },
    { name: "Foxy the Pirate Fox", cost: 500, initialCost: 500, pps: 15, img: "images/foxy.png", sounds: ["sounds/foxy.mp3"], owned: 0 },
    { name: "Freddy Fazbear", cost: 1000, initialCost: 1000, pps: 50, img: "images/freddy.png", sounds: ["sounds/freddy.mp3"], owned: 0 },
    { name: "Golden Freddy", cost: 5000, initialCost: 5000, pps: 100, img: "images/golden.png", sounds: ["sounds/golden1.mp3", "sounds/golden2.mp3"], owned: 0 },
    { name: "The Mimic", cost: 7500, initialCost: 7500, pps: 250, img: "images/mimic.png", sounds: ["sounds/chica.mp3", "sounds/foxy.mp3", "sounds/golden2.mp3", "sounds/mimic.mp3"], owned: 0 },
    { name: "Mangle", cost: 10000, initialCost: 10000, pps: 400, img: "images/mangle.png", sounds: ["sounds/mangle.mp3"], owned: 0 },
    { name: "Withered Bonnie", cost: 15000, initialCost: 15000, pps: 500, img: "images/w_bonnie.png", sounds: ["sounds/wbonnie1.mp3", "sounds/wbonnie2.mp3"], owned: 0 },
    { name: "The Puppet", cost: 20000, initialCost: 20000, pps: 750, img: "images/puppet.png", sounds: ["sounds/puppet.mp3"], owned: 0 },
    { name: "Springtrap", cost: 25000, initialCost: 25000, pps: 1000, img: "images/springtrap.png", sounds: ["sounds/springtrap1.mp3", "sounds/springtrap2.mp3", "sounds/springtrap3.mp3", "sounds/springtrap4.mp3"], owned: 0 }
    ];
...
function buyAnimatronic(index, event) {
    ...
    if (pizzaCount >= animatronic.cost) {
        ...
        if (animatronic.owned === 1) {
            // Pass both img and sounds
            displayAnimatronic(animatronic.img, animatronic.sounds);
        }
...
function displayAnimatronic(imgUrl, sounds) {
    ...
    // Add an event listener for playing a random sound from the list
    animatronicImg.addEventListener("click", () => {
...
function playSound(soundFile) {
    const audio = new Audio(soundFile); // Use the provided sound file
    audio.play();
}
...
function displayBorder(){
    const totalOwned = animatronics.reduce((sum, animatronic) => sum + animatronic.owned, 0);
    if (totalOwned > 0) {
        animatronicDisplay.style.border = "5px solid white"; // Set the border if animatronics are owned
    } else {
        animatronicDisplay.style.border = "0"; // Remove the border if none are owned
    }
}
...
function resetGameState() {
    ...
    pizzaCount = 0;
    pizzasPerSecond = 0;
    slicerCost = 1000;  // Reset slicer cost to initial value
    clickPower = 1;  // Reset click power to initial value

    // Reset animatronics to their initial state
    animatronics.forEach(animatronic => {
        animatronic.cost = animatronic.initialCost; // Reset to initial cost
        animatronic.owned = 0;  // Set owned to 0
    });

    // Clear all animatronic images and update display
    animatronicDisplay.innerHTML = ""; 
    adjustAnimatronicImageHeight(); // Adjust image height after reset

    // Update the visual elements on the page
    displayBorder();
    updateDisplay();
    updateAnimatronicButtons();
    checkSlicerVisibility();  // Ensure slicer visibility is checked after reset

    console.log("Game state has been reset!");
}

// Key combination to reset the game state: Ctrl + Shift + R
